...
- `--metodologia-orange`: #FF7E08 (primary brand, accents, borders)
- `--metodologia-orange-light`: #FF9E42 (hover states)
- `--metodologia-black`: #000000 (text, headings, hero bg)
- `--metodologia-white`: #FFFFFF (text on dark, card backgrounds)
- `--metodologia-light`: #EFEAE4 (body background)
...
FIND element with class="hero":
  IF exists:
    ...
    FIND child element with class="hero-logo":
      VERIFY content contains "metodología" and "<span>_</span>" or similar
...
FIND element: footer:
  IF exists:
    ...
    FIND metodología logo in footer:
      VERIFY contains "metodología<span>_</span>" or similar
