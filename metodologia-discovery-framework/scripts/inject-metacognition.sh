...
# Sofka SAGE / MetodologIA — Agentic PreSales as Service
# © 2026 Sofka Technologies / MetodologIA. All Rights Reserved.
#
...
#
# Tiers:
#   FULL  (~28 lines) → Permanent triad: discovery-conductor, delivery-manager, risk-controller
...
TRIAD_AGENTS=("discovery-conductor" "delivery-manager" "risk-controller")
...
inject_section() {
  ...
  if grep -q "^## Meta-Cognition Protocol" "$file" 2>/dev/null || \
    ...
    if [ "$FORCE" = false ]; then
      ...
      # Remove from section header to next ## header (exclusive)
      local tmp_file
      tmp_file=$(mktemp)
  ...
  # Insert template before the injection line
