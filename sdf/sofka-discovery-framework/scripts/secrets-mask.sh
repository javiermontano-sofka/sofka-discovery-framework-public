...
SECRETS_MAP="$DISCOVERY_DIR/sage-secrets-map.json"
...
while IFS= read -r finding; do
  ...
  COUNTER=$((COUNTER + 1))
  PLACEHOLDER="[SAGE_MASKED_${COUNTER}]"
...
echo ""
echo "✅ Masked $COUNTER secret(s). Map saved to discovery/sage-secrets-map.json"
echo "⚠️  NEVER commit sage-secrets-map.json — it contains original values."
