...
|---------|------|------|--------|
| landing.html | `$SDF/landing.html` | Landing page del plugin SAGE | ~800+ |
...
Los assets HTML definen la identidad visual de SAGE. El Design System v5 establece los tokens de marca, los componentes reutilizables y las reglas de generacion. Cualquier output HTML del framework debe seguir estos patrones para mantener consistencia de marca Sofka.

---

## Sofka Design System v5 — Dark Authority

Filosofia de diseno: **Neo-Swiss + Minto Pyramid + Rule 3-3-3**
- 3 colores primarios: #FF7E08, #FFD700, #1A1A2E
...
| Token | Valor | Uso |
|-------|-------|-----|
| `--orange` | `#FF7E08` | Color primario Sofka, CTAs, acentos |
| `--orange-glow` | `rgba(255, 126, 8, 0.15)` | Glow effect en hover |
| `--orange-bright` | `#FF9E42` | Variante clara para gradientes |
| `--gold` | `#FFD700` | Exito, completado, badges positivos |
...
El exito en Sofka se representa con **amarillo dorado (`#FFD700`)**. Esta regla aplica a todos los outputs: HTML, Markdown, diagramas Mermaid, presentaciones, XLSX.
...
|---------|-------------|-----------|
| Nav | nav, nav-brand, logo-mark | SAGE branding, links de seccion |
| Hero | hero, gradient-text, CTA | Titulo "Agentic PreSales as Service", tagline |
...
| Footer | footer | Version v11.0.0, copyright Sofka |
...
### Gradiente de marca
```css
background: linear-gradient(135deg, #FF7E08, #FFD700);
...
2. **Colores Sofka exclusivamente** — no colores genericos de Bootstrap/Tailwind
...
3. Regla critica: gold (#FFD700) para exito, NUNCA verde bajo ninguna circunstancia
...
- "Que colores usa Sofka?" -> Design System v5: #FF7E08, #FFD700, #1A1A2E
- "Como genero HTML para Sofka?" -> Reglas de generacion + tokens CSS custom properties
- "Que componentes hay?" -> 11: nav, hero, cards, badges, buttons, tables, code, modals, stats, ghost-menu, footer
- "Puedo usar verde para exito?" -> NO, usar #FFD700 (gold). Restriccion absoluta
- "Que font usa Sofka?" -> Inter (Google Fonts), weights 300-900
