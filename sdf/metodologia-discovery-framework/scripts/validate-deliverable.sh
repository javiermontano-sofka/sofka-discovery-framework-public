#!/bin/bash
# MetodologIA Discovery Framework — Deliverable Quality Validator
...
if echo "$CONTENT" | grep -qiE '(Javier Montan|J\. Montano|Montaño)'; then
  ISSUES+=("CRITICAL: Personal name 'Javier Montaño' detected — use 'Javier Montaño · Comunidad MetodologIA' instead")
fi
...
if echo "$CONTENT" | grep -qiE '(Dynamics 365|Business Central)'; then
  ...
  if ! echo "$CONTENT" | grep -qiE '(corrección|correction|historical|descartado|discarded|OUT-)'; then
    WARNINGS+=("'Dynamics 365' or 'Business Central' detected — verify this is correct (OurMetodologIA uses SAP S/4HANA)")
  fi
fi
