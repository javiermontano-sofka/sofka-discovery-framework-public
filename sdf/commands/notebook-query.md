...
# /sdf:notebook-query — Query NotebookLM Knowledge Base

> Diseñado por **Javier Montaño**. Plugin: sdf v3.2+

## ROL

Master: `@research-scientist` (dominante — valida contra sources del notebook)
Soporte: `@discovery-conductor` para consolidar

## OBJETIVO

Consultar un notebook con fuentes ya importadas. A diferencia de `/sdf:consulta` (knowledge del modelo), esto usa **las fuentes curadas del notebook** como ground truth.
...
- `@research-scientist` etiqueta cada afirmación con `[NOTEBOOKLM]` + source ID
...
- Recomendación: si hay gaps → `/sdf:notebook-research` para expandir
...
- Notebook debe tener sources importados (si está vacío → mensaje guía a `/sdf:notebook-research`)
