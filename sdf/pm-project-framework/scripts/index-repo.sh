#!/usr/bin/env bash
# index-repo.sh — Structured Repository Inventory
# PMO-APEX v1.0 — Agentic Project Management as Service
# © 2026 PMO-APEX. All Rights Reserved.
#
# Creates project/repo-index.json with structured file inventory.
...
OUTPUT_FILE="${PROJECT_DIR}/repo-index.json"

# Create .project directory structure
mkdir -p "${PROJECT_DIR}/insights"
mkdir -p "${PROJECT_DIR}/transcripts"
mkdir -p "${PROJECT_DIR}/rag-priming"
...
# Detect PM tools
PM_TOOLS="none"
[[ -f "${TARGET_DIR}/.jira.yml" || -f "${TARGET_DIR}/jira.config" ]] && PM_TOOLS="Jira"
[[ -f "${TARGET_DIR}/azure-boards.yml" ]] && PM_TOOLS="Azure DevOps"
[[ -f "${TARGET_DIR}/.monday.yml" ]] && PM_TOOLS="Monday.com"
[[ -f "${TARGET_DIR}/sprint-backlog.md" ]] && PM_TOOLS="Agile/Scrum"
...
if [[ -d "${TARGET_DIR}/.git" ]]; then
  ...
  FILE_LIST=$(find "$TARGET_DIR" -type f -not -path '*/.git/*' -not -path '*/node_modules/*' -not -path '*/project/*' 2>/dev/null | head -500)
fi
