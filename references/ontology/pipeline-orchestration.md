...
El pipeline de discovery de Sofka SAGE transforma un engagement de pre-venta en un conjunto de entregables técnicos y funcionales con evidencia trazable. Opera en 10 fases secuenciales (0-6 + reportes), controladas por 4 quality gates que actúan como hard stops.
...
flowchart TD
    ...
    style G1 fill:#FFD700,stroke:#1A1A2E,color:#1A1A2E
    style G15 fill:#FFD700,stroke:#1A1A2E,color:#1A1A2E
    style G2 fill:#FFD700,stroke:#1A1A2E,color:#1A1A2E
    style G3 fill:#FFD700,stroke:#1A1A2E,color:#1A1A2E
```
...
|-----------|-------|---------|
| 00 Discovery Plan | `discovery-orchestrator` | `/sdf:generate-plan` |
| 01 Stakeholder Map | `stakeholder-mapping` | `/sdf:map-stakeholders` |

### Fase 1 — Contexto
| Entregable | Skill | Comando |
|-----------|-------|---------|
| 02 Brief Técnico | `input-analysis` | `/sdf:generate-brief` |

### Fase 2 — Diagnóstico
| Entregable | Skill | Comando |
|-----------|-------|---------|
| 03 Análisis AS-IS | `asis-analysis` | `/sdf:diagnose-asis` |
| 04 Mapeo de Flujos | `flow-mapping` | `/sdf:trace-flows` |

### Fase 3 — Evaluación (GATE 1)
| Entregable | Skill | Comando |
|-----------|-------|---------|
| 05 Escenarios ToT | `scenario-analysis` | `/sdf:evaluate-scenarios` |
| 05b Feasibility Think Tank | `multidimensional-feasibility` | `/sdf:validate-feasibility` |

### Fase 4 — Diseño (GATE 1.5, GATE 2)
| Entregable | Skill | Comando |
|-----------|-------|---------|
| 06 Solution Roadmap | `solution-roadmap` | `/sdf:chart-roadmap` |

### Fase 5 — Formalización
| Entregable | Skill | Comando |
|-----------|-------|---------|
| 07 Especificación Funcional | `functional-spec` | `/sdf:write-spec` |
| 08 Pitch Ejecutivo | `executive-pitch` | `/sdf:craft-pitch` |
| 09 Handover Operativo | `discovery-handover` | `/sdf:deliver-handover` |

### Fase 6 — Reportes (GATE 3)
| Entregable | Skill | Comando |
|-----------|-------|---------|
| 10 Deck Ejecutivo | `executive-pitch` | `/sdf:present-findings` |
| 11 Deep-dive Técnico | `output-engineering` | `/sdf:report-tech` |
| 12 Hallazgos Funcionales | `output-engineering` | `/sdf:report-func` |
| 13 Revisión Negocio (INTERNO) | `commercial-model` | `/sdf:review-business` |
| 14 Oportunidades IA | `ai-center-discovery` | `/sdf:discover-ai` |
...
|------|---------|------------|-------|
| `run-guided` | `/sdf:run-guided` | 16 (todos) | 4 con pausa humana |
| `run-auto` | `/sdf:run-auto` | 16 (todos) | 4 con auto-aprobación |
| `run-express` | `/sdf:run-express` | 3 (Plan + ASIS + Roadmap) | G1 simplificado |
| `run-deep` | `/sdf:run-deep` | 7 (arquitectura profunda) | G1 + G2 |
