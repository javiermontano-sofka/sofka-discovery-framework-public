...
Prompts específicos para escenarios de uso frecuentes del skill `sofka-roadmap-poc`. Cada prompt incluye contexto, variables esperadas, y criterios de calidad del output.
...
| `{DURACIÓN}` | 6 |
| `{MODELO_EQUIPO}` | "Provider+Client hybrid, 4 seniors Sofka + 2 SMEs cliente" |
| `{INDUSTRIA}` | "Banca" |
