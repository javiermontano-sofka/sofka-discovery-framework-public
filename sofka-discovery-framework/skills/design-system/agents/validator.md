# Sofka Design System v4 HTML Validator Agent

## Purpose
This agent validates a generated HTML deliverable against the Sofka Design System v4 specification. It performs comprehensive checks on CSS tokens, semantic rules, page structure, typography, accessibility, and responsive design.

## Canonical Token Reference

### Brand Core Colors (Must Match Exactly)
- `--sofka-orange`: #FF7E08 (primary brand, accents, borders)
- `--sofka-orange-light`: #FF9E42 (hover states)
- `--sofka-black`: #000000 (text, headings, hero bg)
- `--sofka-white`: #FFFFFF (text on dark, card backgrounds)
- `--sofka-light`: #EFEAE4 (body background)
- `--sofka-dark`: #B8A894 (muted text)

### Semantic Colors (Critical for v4)
- `--sofka-positive`: #FFD700 (YELLOW - success/checkmarks, NOT green)
- `--sofka-warning`: #D97706 (amber - warnings)
- `--sofka-critical`: #DC2626 (red - errors)
- `--sofka-info`: #2563EB (blue - information)

### Decorative Colors (Charts/Data Viz Only)
- `--sofka-green`: #42D36F (DECORATIVE ONLY - not for semantic states)
- `--sofka-teal`: #06C8C8
- `--sofka-violet`: #9747FF
- `--sofka-pink`: #FE9CAB
...
**Specific checks:**
- Body background: Must be #EFEAE4 or rgba(239, 234, 228, 1)
...
- Checkmarks/success icons: Must be #FF7E08 (orange), NOT #42D36F (green)
- Warning text: If on amber background, must be dark or use black

---

### 2. Sofka Design System v4 Semantic Rule Validation
...
FOR each checkmark or success icon:
  IF color is #FF7E08 (orange):
    PASS
  ELSE IF color is #42D36F (green):
    FLAG as "CRITICAL: Checkmark using green - must be orange (#FF7E08) per v4 spec"
  ELSE:
...
FIND element with class="hero":
  IF exists:
    ...
    VERIFY border-bottom: 8px solid #FF7E08 (orange)
      IF width != 8px: FLAG as "CRITICAL: Hero border-bottom width is [ACTUAL]px, must be 8px"
      IF color != #FF7E08: FLAG as "CRITICAL: Hero border-bottom color is [ACTUAL], must be #FF7E08"

    FIND child element with class="hero-logo":
      VERIFY content contains "sofka" and "<span>_</span>" or similar
...
FOR each element with class="section":
  ...
  FIND section-header child:
    ...
    FIND .section-number child:
      ...
      VERIFY background-color: #000000 (black)
      VERIFY color: #FF7E08 (orange text)
...
FIND element: footer:
  IF exists:
    ...
    VERIFY border-top: 8px solid #FF7E08 (orange)
      IF width != 8px: FLAG as "CRITICAL: Footer border-top width is [ACTUAL]px, must be 8px"
      IF color != #FF7E08: FLAG as "CRITICAL: Footer border-top color is [ACTUAL], must be #FF7E08"
    ...
    FIND sofka logo in footer:
      VERIFY contains "sofka<span>_</span>" or similar
      VERIFY span color: #FF7E08 (orange underscore)
...
FOR each link and button:
  ...
  VERIFY outline color: #FF7E08 (orange)
...
FOR each card variant:
  .card-accent:
    VERIFY border-top: 5px solid #FF7E08
  ...
  .card-highlight:
    VERIFY border: 2px solid #FF7E08
...
FOR each element with class="diagram-box":
  ...
  FIND .diagram-label:
    ...
    VERIFY color: #FF7E08 (orange) or light text color
...
FOR each element with class="badge":
  ...
  FOR base .badge (no variant):
    VERIFY background: #FF7E08 (orange)
    VERIFY color: #FFFFFF (white)

  FOR .badge-outline:
    VERIFY background: transparent
    VERIFY border: 1px solid #FF7E08
    VERIFY color: #FF7E08
