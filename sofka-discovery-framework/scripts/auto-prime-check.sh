...
if [[ "$BASENAME" == "sofka-discovery-framework" ]]; then
...
CALEOF
  ...
  echo "  → Ejecutar /sdf:prime-repo para máxima profundidad"
...
$(if [[ $TOTAL_RAG -lt 3 ]]; then
  echo "- ⚠️ Cobertura baja (${TOTAL_RAG} archivos). Considerar ejecutar \`/sdf:prime-repo\` para ampliar."
...
$(if [[ -n "$UNPRIMED_ATTACHMENTS" ]]; then
  ...
  echo -e "${UNPRIMED_ATTACHMENTS}"
  echo ""
  echo "→ Procesar con \`/sdf:prime-repo\` fase 3 (attachment processing)"
fi)
