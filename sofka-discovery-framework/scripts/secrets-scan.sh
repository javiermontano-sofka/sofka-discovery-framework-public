...
AUDIT_LOG="$DISCOVERY_DIR/.sage-secrets-audit.log"
...
if [ ! -f "$GITIGNORE" ]; then
  echo ".sage-secrets-audit.log" > "$GITIGNORE"
  echo ".sage-secrets-map.json" >> "$GITIGNORE"
elif ! grep -q '.sage-secrets-audit.log' "$GITIGNORE" 2>/dev/null; then
  echo ".sage-secrets-audit.log" >> "$GITIGNORE"
  echo ".sage-secrets-map.json" >> "$GITIGNORE"
fi

# Initialize audit log
echo "=== SAGE G0 Security Scan ===" > "$AUDIT_LOG"
...
if [ "$FINDINGS" -gt 0 ]; then
  ...
  echo "Action: Run /sdf:scan-secrets to review and /sdf:mask-secrets to remediate" >> "$AUDIT_LOG"
  ...
  echo "   Remediate: /sdf:scan-secrets"
  exit 1
else
