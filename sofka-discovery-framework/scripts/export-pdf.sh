...
# ─── Sofka CSS for PDF ───
SOFKA_CSS=$(cat << 'CSSEOF'
<style>
  :root {
    --sofka-primary: #FF7E08;
    --sofka-bg: #EFEAE4;
    --sofka-success: #FFD700;
    --sofka-text: #1A1A1A;
    --sofka-text-light: #4A4A4A;
  }
  body {
    font-family: 'Inter', -apple-system, sans-serif;
    font-size: 11pt;
    line-height: 1.6;
    color: var(--sofka-text);
    background: white;
    max-width: 210mm;
    margin: 0 auto;
    padding: 20mm;
  }
  h1 { color: var(--sofka-primary); font-weight: 700; font-size: 22pt; border-bottom: 3px solid var(--sofka-primary); padding-bottom: 8px; }
  h2 { color: var(--sofka-primary); font-weight: 600; font-size: 16pt; margin-top: 24px; }
  h3 { color: var(--sofka-text); font-weight: 600; font-size: 13pt; }
  table { border-collapse: collapse; width: 100%; margin: 12px 0; }
  th { background: var(--sofka-primary); color: white; font-weight: 600; padding: 8px 12px; text-align: left; }
  td { padding: 6px 12px; border-bottom: 1px solid #ddd; }
  tr:nth-child(even) { background: var(--sofka-bg); }
  code { background: #f5f5f5; padding: 2px 6px; border-radius: 3px; font-size: 0.9em; }
  pre { background: #1A1A1A; color: #f5f5f5; padding: 16px; border-radius: 6px; overflow-x: auto; }
  blockquote { border-left: 4px solid var(--sofka-primary); padding-left: 16px; color: var(--sofka-text-light); font-style: italic; }
  ...
  /* NEVER green for success — always gold */
...
if command -v pandoc >/dev/null 2>&1; then
  ...
  if [ -f "$TEMPLATE_DIR/sofka-pdf-template.typ" ]; then
    typst compile "$INPUT_FILE" "$OUTPUT_FILE" --root "$TARGET_DIR" 2>/dev/null
    echo "✅ PDF exported: $OUTPUT_FILE"
  else
    echo "❌ Typst template not found at $TEMPLATE_DIR/sofka-pdf-template.typ"
    exit 1
  fi
