#!/usr/bin/env bash
# sdf-init.sh — Interactive initialization wizard for Sofka SAGE
...
cat << 'BANNER'
  ...
  ║                                                      ║
  ║   🟠  SOFKA SAGE v12.1                               ║
  ║   Agentic PreSales as Service                        ║
  ║                                                      ║
  ║   Interactive Initialization Wizard                  ║
  ║   © 2026 Sofka Technologies                          ║
  ║                                                      ║
  ╚══════════════════════════════════════════════════════╝
...
# SAGE session files (sensitive, ephemeral)
.sage-secrets-audit.log
.sage-secrets-map.json
session-state.json
GIEOF
...
> Generado por \`sdf-init.sh\` — $(date -u +%Y-%m-%dT%H:%M:%SZ)
...
| **Inicializado por** | sdf-init.sh v12.1 |

## Discovery Status

- **Fase actual**: Pre-discovery (pendiente \`/sdf:run-auto\` o \`/sdf:run-guided\`)
- **Entregables**: 0/16
- **Gates pasados**: 0/4
...
/sdf:run-express       # Go/No-Go en 1 sesión
/sdf:run-deep          # Dirección arquitectónica
\`\`\`
SEOF
...
if bash "$PLUGIN_DIR/scripts/secrets-scan.sh" "$TARGET_DIR" 2>/dev/null; then
  ...
  echo "     Review: .discovery/.sage-secrets-audit.log"
fi
...
echo ""
echo "  🟠 SAGE initialized for ${CLIENT_NAME}"
echo ""
echo "  📋 Configuration:"
...
if [ "$G0_STATUS" = "FINDINGS" ]; then
  echo "     1. Review secrets: /sdf:scan-secrets"
  echo "     2. Then start discovery: /sdf:run-auto or /sdf:run-guided"
else
  echo "     Start discovery: /sdf:run-auto or /sdf:run-guided"
fi
echo ""
echo "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
