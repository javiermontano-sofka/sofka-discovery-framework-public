...
$(if [[ $DELIVERABLE_COUNT -eq 0 ]]; then
  ...
  echo "1. \`/sdf:run-auto\` — Pipeline completo autónomo"
  echo "2. \`/sdf:run-express\` — Go/No-Go rápido (3 entregables)"
  echo "3. \`/sdf:run-guided\` — Pipeline guiado con facilitación"
  ...
  echo ""
  echo "### Próximos pasos recomendados"
  echo "1. \`/sdf:rescue-stalled\` — Diagnosticar y completar"
  echo "2. \`/sdf:audit-quality\` — Auditar lo existente"
  echo "3. \`/sdf:improve-deliverables\` — Evolucionar entregables"
else
  echo "✅ **Pipeline completo** — 16/16 entregables."
  echo ""
  echo "### Próximos pasos recomendados"
  echo "1. \`/sdf:audit-quality\` — Auditoría con scorecard"
  echo "2. \`/sdf:improve-deliverables\` — Evolucionar entregables"
fi)
