...
This skill (v1) provides the module-level configuration reference. The v2 SAP ecosystem expands this into specialized discovery skills:

| Skill | Purpose | Relationship |
|-------|---------|-------------|
| `sofka-sap-discovery` | Pipeline orchestration | Orchestrates all SAP skills including this one |
| `sofka-sap-activate-methodology` | SAP Activate phases & governance | Phase/gate framework |
| `sofka-sap-fit-to-standard` | Workshop facilitation per module | Expands the gap scoring matrix from this skill |
| `sofka-sap-gap-analysis` | Deep gap classification & ADRs | Expands gap management from this skill |
| `sofka-sap-solution-design` | Clean Core target-state architecture | Consumes gaps, produces SAD |
| `sofka-sap-btp-extensibility` | BTP, ABAP Cloud, RAP patterns | Extension patterns for gaps |
| `sofka-sap-data-migration` | Strangler Fig generalized | Generalizes Section 5 of this skill |
| `sofka-sap-integration` | CPI, OData, CATS, Event Mesh | Generalizes Section 6 of this skill |
| `sofka-sap-testing-validation` | UAT, regression, cutover | Testing strategy |
| `sofka-sap-change-adoption` | ADKAR, Enable Now, hypercare | Change management |

**Entry point**: Use `/sdf:sap-discovery` to run the full SAP discovery pipeline.
